...
// FilePaths and DirectoryPaths can be initialized the same as a GenericPAth, but their initializers are failable.
// The initializers will fail if the path exists and does not match the expected type. If the path does not exist, then the object will be created successfully
...
```swift
// Paths conform to the StatDelegate protocol, which means that they use the `stat` utility to gather information about the file (ie: size, ownership, modify time, etc)
