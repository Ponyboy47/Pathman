...
public protocol Readable: Openable, Seekable {
    ...
    func read(from offset: Offset, bytes byteCount: Int64?) throws -> Data
    /// Seeks to the specified offset and returns the specified number of bytes in a string
    func read(from offset: Offset, bytes byteCount: Int64?, encoding: String.Encoding) throws -> String?
}

public extension Readable {
    public func read(from offset: Offset = Offset(from: .current, bytes: 0), bytes byteCount: Int64? = nil, encoding: String.Encoding = .utf8) throws -> String? {
...
extension Open: Readable where PathType == FilePath {
    public func read(from offset: Offset = Offset(from: .current, bytes: 0), bytes byteCount: Int64? = nil) throws -> Data {
        try seek(offset)

        // Either read the specified number of bytes, or read the entire file
        let bytesToRead = byteCount ?? size

        let bSize = bufferSize ?? 0

        if bSize < bytesToRead {
            buffer?.deallocate()
