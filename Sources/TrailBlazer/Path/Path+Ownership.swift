...
public extension Ownable {
    ...
    public func change(owner username: String? = nil, group groupname: String? = nil) throws {
        ...
        if let username = username {
            guard let _uid = try? getUserInfo(username).pw_uid else { throw UserInfoError.getError() }
            uid = _uid
        } else {
            uid = ~0
        }
        ...
        if let groupname = groupname {
            guard let _gid = try? getGroupInfo(groupname).gr_gid else { throw GroupInfoError.getError() }
            gid = _gid
        } else {
            gid = ~0
        }
